...
module.exports = function (app) {
  app.get('/leagues/:league', function (req, res) {
    // var url = 'http://cbkregio-oost.be/index.php?page=competitie&afdeling=' + req.params.league;
    var url = 'https://web.archive.org/web/20120617122959/http://www.cbkregio-oost.be:80/index.php?page=competitie&afdeling=2B';
    ...
    request(url, function (error, response, html) {
      if (!error) {
        ...
        var titleArr = $('.nieuwstbl .nwstitle').text().replace(/[\n\t\r]/g, '').trim().split(' - ');
        ...
        $('.kalendertbl .klassementtbl tr').each(function (i, element) {
          ...
          $(this).find('td:not(.th)').each(function (j, element) {
            var key;
        ...
        $('.kalendertbl .kalendertbl tr').each(function (i, element) {
          $(this).find('td.th').each(function (j, element) {
            if (j === 0) {
              var row = {};    
              var titleArr = $(this).text().trim().split(' - ');

              row.date = moment(titleArr[0], 'DD-MM-YYYY').isValid() ? titleArr[0] : null;
              row.type = titleArr[1];
              row.games = [];

              $(this).parent().attr('data-date', row.date);
        ...
        $('.kalendertbl .kalendertbl tr').each(function (i, element) {
          ...
          $(this).find('td:not(.th)').each(function (j, element) {
            ...
            if (j === 5) {
              var scoresheet = $(this).find('a').attr('href');
