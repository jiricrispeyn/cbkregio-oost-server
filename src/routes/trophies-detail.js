    ...
    request(url, function (error, response, html) {
      if (!error) {
        ...
        $('.bekertbl').each(function (i, element) {
          ...
          $(this).find('tr').each(function(j, element) {
            var content = $(this).find('.content, .contentwhite');
            var contentscore = $(this).find('.contentscore, .contentscorewhite');
            var row = {};
            ...
            $(this).find('td').each(function (k, element) {
              ...
              if (key) {
                ...
                if (team) {
                  ...
                  if (key === 'club') {
                    row[team]['won'] = $(this).find('img').length > 0 ? true : false;
                  }
                } else {
                  row[key] = text;
                }
          ...
          var date = $(this).closest('tr').prev().find('td').text();
